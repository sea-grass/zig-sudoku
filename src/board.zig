...
pub fn Board(comptime N: u32) type {
    const CellNumber = if (N == 3) u64 else unreachable;
    const CellData = union(enum) {
        empty: void,
        puzzle: CellNumber,
        user: CellNumber,
    };
    ...
    return struct {
        ...
        pub fn get_row(self: @This(), row_index: usize) ![]const Cell {
        ...
        pub fn get_col(self: @This(), col_index: usize) ![]const Cell {
            var list = std.ArrayList(Cell).init(self.allocator);
            defer list.deinit();
        ...
        pub fn get_square(self: @This(), square_index: usize) ![]const Cell {
            var list = std.ArrayList(Cell).init(self.allocator);
            defer list.deinit();
        ...
        pub fn newGameFromFile(self: *@This(), file_path: []const u8) !void {
        ...
        test "iterate over board cells" {
            std.debug.print("\n", .{});
            var board = Board(3).init(std.testing.allocator);
